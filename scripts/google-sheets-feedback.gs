 ...
 *
 * Deploy as web app and set GOOGLE_SHEET_FEEDBACK_WEBHOOK_URL in .env
 * After editing this file, create a new Apps Script deployment (Manage deployments → Edit → New version).
 */
...
function doPost(e) {
  try {
    ...
    return jsonResponse({ ok: true, version: FEEDBACK_SHEET_VERSION });
  } catch (err) {
