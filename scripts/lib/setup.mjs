...
const resolveYtDlp = async (repair) => {
  ...
  try {
    ...
    ytdlpPath = constants.YOUTUBE_DL_PATH;
  } catch {
    ytdlpPath = path.join(projectRoot, 'node_modules', 'youtube-dl-exec', 'bin', process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp');
  }
  ...
  if (repair) {
    ...
    await run(npmCommand(), ['rebuild', 'youtube-dl-exec'], { stdio: 'inherit' }).catch(() => undefined);
  }
