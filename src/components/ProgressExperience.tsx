...
type ProgressMode = 'extract' | 'download' | 'convert' | 'audio';
...
export function useSmoothProgress(active: boolean, actualProgress?: number, ceiling = 94, loadedBytes = 0) {
  ...
  return actualProgress === 100 ? 100 : displayProgress;
}
...
export function FriendlyError({ message, retrying }: { message: string; retrying?: boolean }) {
  return (
    <div className="fade-in flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900">
      <AlertCircle className="mt-0.5 h-5 w-5 shrink-0" />
      <div>
        <p className="font-semibold">{retrying ? 'Retrying extraction...' : 'That source needs another route'}</p>
