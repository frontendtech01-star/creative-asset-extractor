...
export const apiFetchWithTimeout = async (path: string, init: RequestInit = {}, timeoutMs = ZIP_DOWNLOAD_TIMEOUT_MS) => {
  ...
  try {
    ...
    if (error?.name === 'AbortError') {
      throw new Error('ZIP creation timed out. Try fewer items or download individually.');
    }
    throw error;
  } finally {
